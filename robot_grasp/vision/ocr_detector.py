...
OCR_OBJECT_CLASSES = ["plastic bag"]  # models/yolo/best.pt 是塑料包模型
...
WARD_DIRECTORY_PATH = os.path.join(PROJECT_ROOT, "configs", "ocr", "ward_directory.json")
...
def _load_ward_directory(path: str = WARD_DIRECTORY_PATH) -> list[dict]:
    """Load OCR ward targets from the imported hospital ward directory.

    The project intentionally does not keep a hardcoded ward fallback. If the
    directory file is missing or empty, ward-only OCR should fail explicitly.
    """
    if not os.path.exists(path):
        raise FileNotFoundError(f"OCR ward directory not found: {path}")
    with open(path, "r", encoding="utf-8") as f:
        payload = json.load(f)
    wards = payload.get("wards", [])
    if not wards:
        raise ValueError(f"OCR ward directory is empty: {path}")
    return wards


_WARD_ENTRIES = _load_ward_directory()
_WARD_LIST = [str(item["canonical"]).strip() for item in _WARD_ENTRIES if str(item.get("canonical", "")).strip()]
_WARD_ALIAS_TO_CANONICAL = {}
for item in _WARD_ENTRIES:
    ...
    for alias in item.get("aliases", []):
        key = _ward_key(alias)
        if key:
            _WARD_ALIAS_TO_CANONICAL[key] = canonical
    _WARD_ALIAS_TO_CANONICAL[_ward_key(canonical)] = canonical


def _ward_canonical(text: str) -> str | None:
    return _WARD_ALIAS_TO_CANONICAL.get(_ward_key(text))

"""
分析 apps/grasp/run_realtime_detection.py 保存的性能采样。

用法:
    python tools/debug/analyze_perf.py
    python tools/debug/analyze_perf.py data/samples/grasp_data_YYYYMMDD_HHMMSS.csv
"""
...
def main():
    ...
    if not path:
        print("没有找到 data/samples/grasp_data_*.csv")
        sys.exit(1)
    ...
    if not rows:
        print("没有 type=perf 的性能采样；请先运行 apps/grasp/run_realtime_detection.py 并按 s 保存。")
        return

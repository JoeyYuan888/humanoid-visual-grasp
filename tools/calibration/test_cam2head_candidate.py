...
from robot_grasp.transforms.coordinate_utils import make_transform
...
def main():
    ...
    parser.add_argument(
        "--latest-csv",
        action="store_true",
        help="Use the latest data/samples/grasp_data_*.csv object_results.",
    )
    parser.add_argument(
        "--csv",
        default=None,
    ...
    if args.latest_csv:
        ...
        if not csv_path:
            print("[!] no data/samples/grasp_data_*.csv found")
        else:
            print(f"\nlatest csv: {csv_path}")
    ...
    if not points:
        print("\n没有输入点。示例:")
        print("  python tools/calibration/test_cam2head_candidate.py --latest-csv")
        print("  python tools/calibration/test_cam2head_candidate.py --point-mm 100 150 1200")
        return

...
from robot_grasp.transforms.coordinate_utils import camera_point_to_base_pose, make_transform
from robot_grasp.common.grasp_flow import object_conf, select_grasp_target, summarize_target
...
DEFAULT_LOCKED_TARGET = os.path.join(PROJECT_ROOT, "data", "runtime", "mpc_locked_target_latest.json")
GRASP_PROFILES = {
    "legacy_no_orientation": os.path.join(PROJECT_ROOT, "data", "poses", "grasp_profile_legacy_no_orientation.json"),
    "tuned_with_orientation": os.path.join(PROJECT_ROOT, "data", "poses", "grasp_profile_tuned_with_orientation.json"),
}
...
def _latest_grasp_csv() -> str | None:
    files = glob.glob(os.path.join(PROJECT_ROOT, "data", "samples", "grasp_data_*.csv"))
...
def _resolve_existing_path(path: str | None) -> str | None:
    ...
    if not os.path.isabs(path):
        ...
        candidates.append(os.path.join(PROJECT_ROOT, "data", "poses", os.path.basename(path)))
        candidates.append(os.path.join(PROJECT_ROOT, "data", "runtime", os.path.basename(path)))
        candidates.append(os.path.join(PROJECT_ROOT, "data", "calibration", os.path.basename(path)))
...
def _apply_grasp_profile(args) -> None:
    ...
    if args.orientation_file is None and not args.stop_at_last_via:
        args.orientation_file = _resolve_existing_path(profile.get("orientation_file"))
...
def main():
    ...
    parser.add_argument("--save-target", nargs="?", const=DEFAULT_LOCKED_TARGET, default=None,
                        help="Save computed BASE target to this JSON path; default path is data/runtime/mpc_locked_target_latest.json.")
    ...
    parser.add_argument("--orientation-file", default=None,
                        help="JSON saved by tools/capture/capture_mpc_pose.py; uses its orientation.")
    ...
    parser.add_argument("--via-file", action="append", default=None,
                        help="JSON saved by tools/capture/capture_mpc_pose.py; appends a full pose waypoint. Can be repeated.")
    ...
    if locked:
        ...
        if not csv_path:
            print("[✗] 没找到 data/samples/grasp_data_*.csv，请先运行 apps/grasp/run_realtime_detection.py 并保存一次")
            raise SystemExit(1)
        ...
        if target is None:
            ...
            print("    可尝试降低 --preferred-label 限制，或重新运行 apps/grasp/run_realtime_detection.py 保存更稳定的数据")
            raise SystemExit(1)
        ...
        if args.via_file:
            for path in args.via_file:
                ...
                if joint_states is not None:
                    ...
                    if state is None:
                        print(f"[✗] {path} 中没有 mpc_state。请用 tools/capture/capture_mpc_pose.py --include-joints 重新记录")
                        raise SystemExit(1)

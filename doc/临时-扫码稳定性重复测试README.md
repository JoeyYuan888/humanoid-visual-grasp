...
```text
data/qr_multiframe_debug/latest_recover/
```
...
```bash
cat data/post_grasp_qr_latest.json
```
...
```bash
python tools/debugs/debug_qr_multiframe_recover.py \
  --input-dir data/qr_multiframe_debug/latest_raw/raw \
  --output-dir data/qr_multiframe_debug/latest_recover \
  --count 5 \
  --fast \
  --no-wechat \
  --min-consensus 2
...
data/qr_multiframe_debug/latest_recover/report.json
```

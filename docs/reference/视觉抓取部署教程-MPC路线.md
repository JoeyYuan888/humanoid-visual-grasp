...
运行 apps/grasp/run_perception_lock.py
运行 apps/grasp/run_visual_grasp_test.py
运行 apps/grasp/run_grasp_flow.py
```
...
```text
--skip-lock          使用已有 data/runtime/mpc_locked_target_latest.json
...
6. 保存 data/runtime/mpc_locked_target_latest.json
...
```bash
conda activate detect
python apps/grasp/run_perception_lock.py --ws-url ws://192.168.20.102:9091
```
...
```bash
python tools/debug/analyze_perf.py
```
...
```bash
python apps/grasp/run_realtime_detection.py --ws-url ws://192.168.20.102:9091
```

窗口里确认检测到塑料袋后按 `s` 保存。低头姿态不要动，再把最新 CSV 中的目标锁存成 BASE 坐标：

```bash
python apps/grasp/run_visual_grasp_test.py --ws-url ws://192.168.20.102:9091 --save-target
```
...
```bash
python tools/debug/run_mpc_points_dry_run.py --ws-url ws://192.168.20.102:9091 --dx 0.01 --execute
```
...
```bash
python tools/capture/capture_mpc_pose.py --ws-url ws://192.168.20.102:9091 --arm right --output data/poses/mpc_grasp_pose_right.json
```
...
```text
--orientation-file data/poses/mpc_grasp_pose_right.json
```
...
```bash
python apps/grasp/run_visual_grasp_test.py --ws-url ws://192.168.20.102:9091 --use-locked-target --orientation-file data/poses/mpc_grasp_pose_right.json --via-point 0.30 -0.30 0.95 --step-distance 0.05 --max-motion 0.05
```

继续朝锁存目标小步移动：

```bash
python apps/grasp/run_visual_grasp_test.py --ws-url ws://192.168.20.102:9091 --use-locked-target --orientation-file data/poses/mpc_grasp_pose_right.json --step-distance 0.15 --max-motion 0.15 --execute --confirm-target
```

实测手臂至少需要上抬约 `18cm` 才能明显超过桌沿，因此当前脚本默认 `--safe-travel-z=0.95`。如果现场仍不够高，继续显式加大：

```bash
python apps/grasp/run_visual_grasp_test.py --ws-url ws://192.168.20.102:9091 --use-locked-target --safe-travel-z 1.00 --step-distance 0.15 --max-motion 0.15 --execute --confirm-target
```

如果 15cm 成功，再逐步增加，例如：

```bash
python apps/grasp/run_visual_grasp_test.py --ws-url ws://192.168.20.102:9091 --use-locked-target --step-distance 0.20 --max-motion 0.20 --execute --confirm-target
```
...
| 视觉检测 | 已完成 | `models/yolo/best.pt` 可识别塑料袋 |
...
| MPC points dry-run | 已完成 | `tools/debug/run_mpc_points_dry_run.py` 能打印 request |
...
```bash
python tools/debug/debug_mpc_interfaces.py
python tools/debug/run_mpc_points_dry_run.py
...
```bash
python apps/grasp/run_realtime_detection.py
python tools/debug/debug_vision_pipeline.py
...
tools/calibration/test_cam2head_candidate.py
tools/calibration/debug_handeye_sources.py
tools/calibration/collect_cam2head_pairs.py
tools/calibration/solve_cam2head.py
data/samples/
data/calibration/
```
...
```bash
python tools/debug/run_mpc_points_dry_run.py --ws-url ws://<机器人IP>:9091 --dx 0.01
```
...
当前配置在 `robot_grasp/common/config.py`：
...
```bash
python tools/calibration/solve_cam2head.py data/samples/pairs.csv
```
...
```bash
python tools/capture/capture_mpc_pose.py --ws-url ws://192.168.20.102:9091 --arm right --include-joints --output data/poses/mpc_via0_home_right.json
python tools/capture/capture_mpc_pose.py --ws-url ws://192.168.20.102:9091 --arm right --include-joints --output data/poses/mpc_via1_pose_right.json
python tools/capture/capture_mpc_pose.py --ws-url ws://192.168.20.102:9091 --arm right --include-joints --output data/poses/mpc_via2_pose_right.json
python tools/capture/capture_mpc_pose.py --ws-url ws://192.168.20.102:9091 --arm right --include-joints --output data/poses/mpc_via3_pose_right.json
```
...
```bash
python tools/debug/run_mpc_points_dry_run.py --execute
```
...
```text
1. 读取最新 data/samples/grasp_data_*.csv 的 object_results
2. 默认选择 label=plastic bag 且 3D 有效、置信度最高的目标
3. 使用 data/calibration/cam2head_vendor_new_20260803.json 做 camera -> HEAD
4. 从 /tf 采样 BASE -> HEAD
...
```bash
python apps/grasp/run_visual_grasp_test.py --target-idx 1
```

如果要用指定 CSV：

```bash
python apps/grasp/run_visual_grasp_test.py --csv data/samples/grasp_data_20260727_133848.csv
```

第一轮真实执行只能做零位移或极小位移验证：

```bash
python tools/debug/run_mpc_points_dry_run.py --dz 0.01 --execute
```
...
```bash
python apps/grasp/run_visual_grasp_test.py --ws-url ws://192.168.20.102:9091 --use-locked-target --step-distance 0.03
```

实机验证阶段不要一次执行完整视觉目标。先使用 `--step-distance`，让末端只朝完整视觉目标方向移动一小段：

```bash
python apps/grasp/run_visual_grasp_test.py --ws-url ws://192.168.20.102:9091 --use-locked-target --step-distance 0.03 --execute --confirm-target
```
...
```bash
python apps/grasp/run_visual_grasp_test.py --use-locked-target --include-descend --offset-z 0.35
```
...
```bash
python tools/debug/run_mpc_points_dry_run.py --ws-url ws://192.168.20.102:9091 --orientation-preset doc-grasp --execute
```

如果姿态方向正确，再在高位小步接近时带上姿态。推荐使用现场保存的姿态文件：

```bash
python apps/grasp/run_visual_grasp_test.py --ws-url ws://192.168.20.102:9091 --use-locked-target --orientation-file data/poses/mpc_grasp_pose_right.json --step-distance 0.05 --max-motion 0.05 --execute --confirm-target
```
...
3. 在电脑端运行 tools/debug/debug_mpc_interfaces.py，确认 `mpc_target` 可通过 rosbridge 解析
4. 确认 /wa/points_seq_tracking 能 execute 一个零位移或极小位移任务
5. 用尺子/工装验证厂商 CAM2HEAD 候选矩阵的绝对误差
6. 用 robot_grasp/transforms/coordinate_utils.py 跑通 camera -> BASE/MPC target

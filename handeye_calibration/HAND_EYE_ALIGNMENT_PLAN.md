...
| 视觉 3D 点 | `robot_grasp/depth_utils.py` 反投影输出 |
...
```text
handeye_calibration/calibration/vendor_handback_board_20260729.json
```
...
python handeye_calibration/collect_vendor_board_cam2head.py \
...
5. 按 q 退出，CSV 会保存在 handeye_calibration/data/。
```

求解命令：

```bash
python handeye_calibration/solve_vendor_board_cam2head.py \
  handeye_calibration/data/vendor_board_cam2head_samples_xxx.csv
```

输出：

```text
handeye_calibration/calibration/cam2head_vendor_board_xxx.json
```
...
```bash
python handeye_calibration/solve_cam2head.py handeye_calibration/data/cam2head_pairs_xxx.csv
```

解算并保存：

```text
handeye_calibration/calibration/cam2head_xxx.json
```
...
```bash
python handeye_calibration/collect_cam2head_pairs.py
```
...
```bash
python handeye_calibration/debug_handeye_sources.py
```
